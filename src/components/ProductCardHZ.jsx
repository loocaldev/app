...
const ProductCardHZ = forwardRef(({ product }, ref) => {
  ...
  useEffect(() => {
    if (hasVariations) {
      ...
      product.variations.forEach((variation) => {
        Object.entries(variation.attribute_data).forEach(
          ([attrName, options]) => {
  ...
  const getMatchingVariation = (attributes) => {
    return product.variations.find((variation) =>
      Object.entries(attributes).every(([attrName, attrValue]) =>
        variation.attribute_data[attrName]?.some(
          (opt) => opt.name === attrValue
        )
      )
    );
  };
  ...
  const syncAttributesFromMaduracion = (updatedAttributes) => {
    ...
    if (!validVariation) {
      ...
      const validCultivo = Array.from(availableOptions["Cultivo"]).find(
        (option) =>
  ...
  const syncAttributesFromCultivo = (updatedAttributes) => {
    ...
    if (!validVariation) {
      ...
      const validMaduracion = Array.from(availableOptions["Maduración"]).find(
        (option) =>
  ...
  const selectedVariation =
    getMatchingVariation(selectedAttributes) || product.variations[0];
  ...
  const priceWithPromotion =
    selectedVariation?.final_price || product.final_price || unitPrice;
  ...
  const discountPercentage =
    isOnPromotion && priceWithPromotion
            ...
            {hasVariations && (
              <div className={styles["product-variables"]}>
                {Object.keys(availableOptions).map((attributeName) => (
                  <div
                    key={attributeName}
                    className={styles["product-variable"]}
                  >
        ...
        <div className={styles["product-box-second"]}>
          <div className={styles["product-price"]}>
            ...
            <div className={styles["product-priceMain"]}>
              ...
              {/* Esta valor no se esta mostrando correctamente para productos variables. Recuerda que este valor es dinamico y cambio con la cantidad agregada al carrito. Por ejemplo, si un producto se vende por 500 gr y se agrega tres, serian 1.5 Kg */}
            </div>

            <div className={styles["product-action"]}>
              <div
                className={classNames(styles["product-addButton"], {
                  [styles["adding-animation"]]: isAdding,
                })}
              >
                {productInCart ? (
                  <>
                  ...
                  <div
                    ...
                    className={styles["addButton-iconCompra"]}
                  >
                    <FiShoppingCart />
                    <span>Añadir</span>
                  </div>
