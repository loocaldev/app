// ProductCardSQRead.jsx
import React, { useState } from "react";
import styles from "../styles/ProductCardSQRead.module.css";
...
function ProductCardSQRead({ product }) {
  const { addToCart, decrementQuantity, removeFromCart, convertQuantity } = useCart();
  ...
  const hasVariations = product.is_variable && product.variations && product.variationId;
  
  // Obtener la variación seleccionada (usada desde el carrito)
  const selectedVariation = hasVariations 
    ? product.variations.find((variation) => variation.id === product.variationId)
    : null;

  // Calcular la cantidad total y el precio total basado en la cantidad en el carrito
  const productQuantity = product.quantity || 1;
  const unitPrice = selectedVariation?.final_price || selectedVariation?.price || product.price;
  ...
  const discountPercentage = isOnPromotion && selectedVariation
    ? `${Math.round((1 - (selectedVariation.final_price / selectedVariation.price)) * 100)}%`
    : null;
  const originalPrice = formatPriceToCOP(selectedVariation?.price || product.price);
        ...
        <div className={styles["product-info"]}>
          ...
          {hasVariations && selectedVariation?.attribute_data && (
            <div className={styles["product-variables"]}>
              {Object.entries(selectedVariation.attribute_data).map(([attrName, options]) => (
...
export default ProductCardSQRead;
