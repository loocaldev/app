  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!validateEmail(email)) {
        ...
        setErrorMessage("Asegúrate de que tu contraseña cumpla con los requisitos de seguridad.");
      } else if (!termsChecked || !privacyChecked) {
        setErrorMessage("Debes aceptar los términos y condiciones y la política de privacidad.");
      } else {
        // Intentar registrar el usuario
        const result = await register(email, password);
  
        if (result?.error) {
          setRegistrationError(result.error); // Muestra el error devuelto
        } else {
      ...
      setRegistrationError("Error inesperado. Intenta nuevamente.");
    } finally {
      setLoading(false);
    }
  };
